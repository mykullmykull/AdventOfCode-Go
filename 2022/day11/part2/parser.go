...
func parse(input []string) monkeys {
	...
	for _, line := range input {
		...
		if line[:len(itemsPrefix)] == itemsPrefix {
			itemStartValues := strings.Split(strings.Split(line, itemsPrefix)[1], ", ")
			for _, startStr := range itemStartValues {
				m.items = append(m.items, item{
					start: utils.StrToInt(startStr),
					ops:   []operation{},
				})
			}
			continue
		...
		if line[:len(testPrefix)] == testPrefix {
			...
			m.divisor = utils.StrToInt(divisibles[0])
			continue
		}
		...
		if line[:len(truePrefix)] == truePrefix {
			...
			m.modMonkeyId = utils.StrToInt(ids[0])
			continue
		}
		...
		if line[:len(falsePrefix)] == falsePrefix {
			...
			m.notMonkeyId = utils.StrToInt(ids[0])
			continue
		}
