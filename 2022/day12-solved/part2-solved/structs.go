...
func (g grid) getStartingSpots() []spot {
	startingSpots := []spot{}
	for x, row := range g.rows {
		for y, elev := range row {
			if elev == 'a' || elev == 'S' {
				...
				// getElevation to currect for 'S'
				newElev, err := g.getElevation(s)
				if err != nil {
					panic(fmt.Sprintf("failed to get elev for spot %v, %v", s, err))
				}
				s.elevation = newElev
				startingSpots = append(startingSpots, s)
			}
		}
	}
...
func (g grid) copyRows(other grid) grid {
	rows := make([]string, len(other.rows))
	for x, row := range other.rows {
		rows[x] = row
	}
	g.rows = rows
	return g
