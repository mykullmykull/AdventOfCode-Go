...
func (c crater) proposeMoves() (crater, int) {
	moves := 0
	for y, row := range c.grid {
		for x, cell := range row {
			if cell == '#' {
				moved := false
				c, moved = c.maybeProposeMove(y, x)
				if moved {
					moves++
				}
			}
...
func (c crater) maybeProposeMove(y int, x int) (crater, bool) {
...
func (c crater) proposeMove(block []string, y int, x int) (crater, bool) {
	nw := string(block[0][0])
	nn := string(block[0][1])
	ne := string(block[0][2])
	ww := string(block[1][0])
