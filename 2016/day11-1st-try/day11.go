...
func (c *Component) matches(c2 Component) bool {
	return c.element == c2.element &&
		c.isGenerator == c2.isGenerator &&
		c.floor == c2.floor
}
...
func (s *Snapshot) isDuplicate(snapshots []Snapshot) bool {
	for _, snapshot := range snapshots {
		if s.elevator == snapshot.elevator {
			if len(s.components) != len(snapshot.components) {
				return false
			}

			for _, component := range s.components {
				doesComponentMatch := false
				for _, component2 := range snapshot.components {
					if component.matches(component2) {
						doesComponentMatch = true
					}
				}
				// component doesn't match any in snapshot
				if !doesComponentMatch {
					return false
				}
			}
			// all components found a match
			return true
		}
	}
	// no snapshot matched
	...
	for _, snapshot := range possibleSnapshots {
		...
		for _, load := range possibleLoads {
			...
			for _, newFloor := range newFloors {
				...
				if isValid(newSnapshot) && !newSnapshot.isDuplicate(newPossibleSnapshots) {
					// fmt.Printf("adding new snapshot: %v\n", newSnapshot)
...
func isValid(snapshot Snapshot) bool {
	// fmt.Print("checking if snapshot is valid\n")
	// printSnapshot(snapshot)
	for _, component := range snapshot.components {
		if isFried(component, snapshot.components) {
			// fmt.Printf("component is fried: %v\n", component)
			return false
		}
	}
	return true
